...
function EditCategory({ categoryDetails, isOpen, onClose, refetch }) {
  ...
  React.useEffect(() => {
    ...
    if (categoryDetails) {
      reset({
        ...
        sn_number: categoryDetails.sn_number || 0,
      });
    }
  ...
  return (
    <Modal
      title={<Title level={3}>{categoryDetails?.name} Edit - Category</Title>}
      open={isOpen}
      onCancel={onClose}
      footer={null}
      centered
    >
      <Form layout="vertical" onFinish={handleSubmit(onSubmit)}>
        ...
        {/* sn_number */}
        <Form.Item label="Sirial Number">
          <Controller
            name="sn_number"
            defaultValue={categoryDetails?.sn_number}
            control={control}
            rules={{ required: "Sirial Number is required" }}
            render={({ field }) => (
              <InputNumber
                min={0}
                className="w-full"
                placeholder="Enter Sirial Number..."
                {...field}
              />
            )}
          />
